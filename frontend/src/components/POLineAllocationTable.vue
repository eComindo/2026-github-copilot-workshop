    ...
    <div v-else class="table-scroll">
      <table class="line-table">
        ...
        <tbody><tr v-for="(row, index) in tableRows" :key="row.id" class="line-row">
          ...
          <td><input v-model.number="row.orderQty" type="number" min="0" :max="row.remainingQty || row.openQty" class="mini-input" aria-label="Order quantity" /></td>
          <td><input v-model="row.deliveryDate" type="date" class="mini-input date-input" aria-label="Delivery date" /></td>
          <td><input v-model.number="row.unitPrice" type="number" min="0" class="mini-input" aria-label="Unit price" /></td><td>{{ formatCurrency(row.orderQty * row.unitPrice) }}</td>
          <td v-if="legacyMode"><button type="button" class="remove-btn" @click="$emit('remove-line', index)">Remove</button></td>
        </tr></tbody>
      </table>
    </div>
  </div>
